...
void ColorBar::get_rgb(float f01, double& r, double& g, double& b)//已知浮点数，求其rgb值
{
    float t;
    int st = 0;
...
void Imagesc::draw()
{
    ...
    if (show_red_line)
    {
        ...
        glLineWidth(2.0);
        glBegin(GL_LINES);
        glVertex2i(red_line_x, 0);
        glVertex2i(red_line_x, app->samples);
        glEnd();
    }
    // if(plt->draw_mouse_line)
    // {
    //     glColor3f(0.0,1.0,0.0);
    //     glLineWidth(2.0);
    //     glBegin(GL_LINES);
    //     glVertex2i(red_line_x-2,plt->smp);
    //     glVertex2i(red_line_x+2,plt->smp);
    //     glEnd();
    // }
}
...
int Imagesc::handle(int event)
{
    int ret = Fl_Gl_Window::handle(event);
    int px, py;
    px = app->trace_s * (Fl::event_x() + 1) / w();px = (px == 0) ? 1 : px;
    py = app->samples * (Fl::event_y() + 1) / h();py = (py == 0) ? 1 : py;
    switch (event)
    {
    case FL_MOVE:
...
void Plot::draw()
{
    ...
    glViewport(0, 0, w(), h());
    glLoadIdentity();
    glOrtho(1, app->samples, p_min, p_max, -1, 1);
    glColor3f(0.0, 0.0, 0.0);
    glBegin(GL_LINE_STRIP);
    for (int x = 0;x < app->samples;x++)
        glVertex2f(x, app->seis[(evx_app - 1) * app->samples + x]);
    glEnd();
    if (draw_mouse_line)
    {
        glColor3f(0.0, 1.0, 0.0);
        glBegin(GL_LINES);
        glVertex2f(smp, p_min);
        glVertex2f(smp, p_max);
        glEnd();
        // ims->redraw();
    }
}
...
int Plot::handle(int event)
{
    ...
    case FL_MOVE:
        char note[128];
        smp = app->samples * (Fl::event_x() + 1) / w();
        smp = (smp == 0) ? 1 : smp;
        sprintf(note, "Sample:%d,Amp:%f", smp, app->seis[(evx_app - 1) * app->samples + smp]);
...
HeaderWin::HeaderWin(int W, int H, const char* title) :Fl_Window(W, H, title)
{
    ...
    vht = new VHeaderTable(500, 70, 140, 580, "Volume Header");
    ...
    hdr_text = new Fl_Text_Editor(670, 30, 510, 580, "Text Header");
    ...
    Fl_Button* save_but = new Fl_Button(670, 620, 100, 25, "save as");
    save_but->callback([](Fl_Widget*, void*)
        {
            int i, j;
            Fl_Native_File_Chooser native;
            native.title("Save As");
            native.filter("SEGY\t*.{sgy,segy}");
            ...
            default:
                ...
                char asc_out[3200];
                for (int k = 0;k < 3200;k++)asc_out[k] = 32;
                int tb_len = hdrwin->hdr_text->buffer()->length();
                unsigned char ebc_out[3200] = { 0 };
                for (i = 0, j = 0;i < tb_len && j < 3200;i++)
                {
                    if (hdrwin->tbuff->text()[i] == 10)j += 80 - j % 80;
                    else asc_out[j++] = hdrwin->tbuff->text()[i];
                }
                for (i = 0;i < 3200;i++)ebc_out[i] = a2e[(unsigned char)asc_out[i]];
                fwrite(ebc_out, 1, 3200, app->fpo);
                fwrite(app->hdr + 3200, 1, 400, app->fpo);
                fseeko64(app->fpi, 3600, 0);
                unsigned char* tbuf = (unsigned char*)malloc(240 + 4 * app->samples);
                hdrwin->begin();
                Progress* progress = new Progress(800, 620, 300, 25);
                hdrwin->end();
                p100 = 0;
    ...
    Fl_Button* export_hdr = new Fl_Button(20, 560, 100, 30, "输出选中道头");
    Fl_Button* load_hdr = new Fl_Button(130, 620, 180, 30, "选中道头替换为参考txt文件");
    app->input_load_hdr = new Fl_Input(240, 580, 200, 30, "参考txt/sgy文件:");
    Fl_Button* open_but = new Fl_Button(445, 580, 40, 30, "@fileopen");
    Fl_Button* copy_hdr_sgy = new Fl_Button(330, 620, 160, 30, "替换道头为参考sgy文件");
    open_but->callback([](Fl_Widget*, void*)
        {
            Fl_Native_File_Chooser native;
            native.title("打开参考文件txt/sgy");
            native.type(Fl_Native_File_Chooser::BROWSE_FILE);
            native.filter("Text or SEGY\t*.{txt,sgy,segy}");
            switch (native.show())
            {
            case -1:break;
            case 1:fl_beep();break;
    ...
    export_hdr->callback([](Fl_Widget*, void*)
        {
            ...
            default:
                ...
                fseeko64(app->fpi, 3600, 0);
                hdrwin->begin();
                Progress* progress = new Progress(150, 550, 240, 25);
                hdrwin->end();
                for (long long tr = 0;tr < app->traces;tr++)
                {
                    check_progress(progress, tr, app->traces);
    ...
    load_hdr->callback([](Fl_Widget*, void*)
        {
            ...
            default:
                FILE* fp, * fps;
                if (istxt1sgy2(app->input_load_hdr->value()) != 1)
                {
                    fl_message("请重新选择txt格式的参考文件喵");
                    return;
                }
                if (NULL == (fp = fl_fopen(app->input_load_hdr->value(), "r")))
                    fl_message("未找到参考文件");
                if (NULL == (fps = fl_fopen(app->fc->filename(), "wb")))
                    fl_message("输出路径不存在，请手动创建该路径");
                int hdrv, hdr_loc;
    ...
    copy_hdr_sgy->callback([](Fl_Widget*, void*)
        {
            if (0 == strcmp(app->input_load_hdr->value(), ""))
            {
                fl_message("请输入参考文件路径喵!");
            ...
            default:
                FILE* fp, * fps;
                if (istxt1sgy2(app->input_load_hdr->value()) != 2)
                {
                    fl_message("请重新选择sgy格式的参考文件喵");
                    return;
                }
                if (NULL == (fp = fl_fopen(app->input_load_hdr->value(), "rb")))
                    fl_message("未找到参考文件");
                if (NULL == (fps = fl_fopen(app->fc->filename(), "wb")))
                    fl_message("输出路径不存在，请手动创建该路径");
                // long long ls;
                ...
                Progress* progress = new Progress(150, 550, 240, 25);
                hdrwin->end();
                unsigned char shdr[3600];
                fread(shdr, 1, 3600, fp);
                int s_samples;
                if (shdr[3224] == 0)s_samples = shdr[3221] + shdr[3220] * 256;
                else s_samples = shdr[3221] * 256 + shdr[3220];
                if (s_samples != app->samples)
                {
                    fl_message("参考文件的道头样点数与当前文件不匹配喵!");
                    fclose(fp);fclose(fps);
                    return;
                }
                unsigned char sbuf[240];
                unsigned char* buf = (unsigned char*)malloc(4 * s_samples);
                fwrite(app->hdr, 1, 3600, fps);
                fseeko64(app->fpi, 3600, 0);
                fseeko64(fp, 3600, 0);
                for (long long tr = 0;tr < app->traces;tr++)
                {
                    ...
                    fread(buf, 1, 4 * s_samples, app->fpi);
                    fread(sbuf, 1, 240, fp);
                    fseeko64(fp, 4 * s_samples, 1);

                    fwrite(sbuf, 1, 240, fps);
                    fwrite(buf, 1, 4 * s_samples, fps);
                }
                hdrwin->remove(progress);
                hdrwin->redraw();
                fclose(fp);fclose(fps);
...
App::App(int X, int Y, int Width, int Height, const char* title) :Fl_Window(X, Y, Width, Height, title)
{
    ...
    menus->add("Tools/Headers", 0, [](Fl_Widget*, void*)
        {
            if (hdrwin == NULL)hdrwin = new HeaderWin(660, 660, "Headers");
    ...
    PlotWin->begin();
    {
        ...
        Fl_Button* plt_export = new Fl_Button(600, 200, 100, 25, "导出到out.txt");
        plt_export->callback([](Fl_Widget*, void*)
            {
                char outfilename[256];
                sprintf(outfilename, "%sout.txt", app->filenames.path_slash);
                FILE* fpt;
                if (NULL == (fpt = fl_fopen(outfilename, "w")))
                {
                    fl_message("路径不存在,请手动创建该路径");
                    return;
                }
                for (int i = 0;i < app->samples;i++)
                    fprintf(fpt, "%g\n", app->seis[(plt->evx_app - 1) * app->samples + i]);
                fclose(fpt);
            });
    }
    PlotWin->set_non_modal();
    PlotWin->callback([](Fl_Widget*, void*) {ims->show_red_line = false;app->PlotWin->hide();ims->redraw();plt->draw_mouse_line = false;});
    PlotWin->hide();
}
...
void expand_right_menu(int evx, int evy, char* num)//num为右键菜单标题
{
    MenuData* data = new MenuData{ evx,evy };
    ...
    right_menu1.add("该列数据", 0, [](Fl_Widget*, void* userdata)
        {
            if (plt != NULL)Fl::delete_widget(plt);
            ...
            for (int i = 0;i < app->samples;i++)
            {
                ...
                plt->p_min = (app->seis[(data->evx - 1) * app->samples + i] < plt->p_min) ? app->seis[(data->evx - 1) * app->samples + i] : plt->p_min;
            }
            plt->redraw();
            delete data;
        }, data);
...
int istxt1sgy2(const char* path)
{
    ...
    if (strcmp(suffix, ".txt") == 0 || strcmp(suffix, ".TXT") == 0)return 1;
    if (strcmp(suffix, ".sgy") == 0 || strcmp(suffix, ".SGY") == 0)return 2;
    if (len >= 5)
    {
        suffix = path + len - 5;
        if (strcmp(suffix, ".segy") == 0 || strcmp(suffix, ".SEGY") == 0)return 2;
    }
    return 0;
}
